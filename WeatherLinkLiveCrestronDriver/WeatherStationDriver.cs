...
public sealed class WeatherStationDriver : ReflectedAttributeDriverEntity
	...
	private ConfigurationItemErrors ApplyConfigurationItems (
		...
		string openWeatherApiKey = GetString (values, "OpenWeatherApiKey");
		string weatherLinkLiveHost = GetString (values, "WeatherLinkLiveHost");
		string units = NormalizeUnits (GetString (values, "Units"));
		int refreshInterval = GetInteger (values, "RefreshIntervalSeconds") ?? DefaultRefreshIntervalSeconds;
		...
		if (units == null)
			{
			errors["Units"] = "Units must be Metric or Imperial.";
			}
