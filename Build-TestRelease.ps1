...
param(
    ...
    [string] $Package = 'WeatherLinkLiveCrestronDriver.ProcessorTests'
)
$ErrorActionPreference = 'Stop'
Set-StrictMode -Version Latest
$root = $PSScriptRoot
if ($Package -ne 'WeatherLinkLiveCrestronDriver.ProcessorTests') { throw 'Unsupported release package.' }
$projectDirectory = Join-Path $root $Package
foreach ($project in @('WeatherLinkLiveCrestronDriver.Tests/WeatherLinkLiveCrestronDriver.Tests.csproj', 'WeatherLinkLiveCrestronDriver.Lifecycle.Tests/WeatherLinkLiveCrestronDriver.Lifecycle.Tests.csproj')) {
    $legacy = $project -like 'WeatherLinkLiveCrestronDriver.Tests/*'
...
& "$root/tools/Test-DiscoveredCoverage.ps1" -Stage Package -SdkRoot $SdkRoot -PackageAssembly "$extracted/$Package.dll" -SourceInventory "$root/artifacts/test-results/WeatherLinkLiveCrestronDriver.Lifecycle.Tests/inventory.json" -ResultsDirectory "$root/artifacts/validation"
